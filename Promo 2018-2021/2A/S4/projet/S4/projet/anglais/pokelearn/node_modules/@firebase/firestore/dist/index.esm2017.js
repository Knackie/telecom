...
class LocalSerializer {
    ...
    toDbTarget(queryData) {
        ...
        const dbLastLimboFreeTimestamp = this.toDbTimestamp(queryData.lastLimboFreeSnapshotVersion);
        let queryProto;
...
class LocalStore {
    ...
    persistence, queryEngine, initialUser) {
         ...
         */
        this.queryDataByTarget = new SortedMap(primitiveComparator);
        /** Maps a query to its targetID. */
        this.targetIdByQuery = new ObjectMap(q => q.canonicalId());
        /**
    ...
    getLastRemoteSnapshotVersion() {
        return this.persistence.runTransaction('Get last remote snapshot version', 'readonly-idempotent', txn => this.queryCache.getLastRemoteSnapshotVersion(txn));
    }
    /**
        ...
        return this.persistence
            ...
            forEachNumber(remoteEvent.targetChanges, (targetId, change) => {
                ...
                if (resumeToken.length > 0) {
                    ...
                    newQueryDataByTargetMap = newQueryDataByTargetMap.insert(targetId, newQueryData);
    ...
    notifyLocalViewChanges(viewChanges) {
        for (const viewChange of viewChanges) {
            ...
            if (!viewChange.fromCache) {
                ...
                assert(queryData !== null, `Can't set limbo-free snapshot version for unknown target: ${targetId}`);
                // Advance the last limbo free snapshot version
                const lastLimboFreeSnapshotVersion = queryData.snapshotVersion;
                const updatedQueryData = queryData.withLastLimboFreeSnapshotVersion(lastLimboFreeSnapshotVersion);
    ...
    allocateQuery(query) {
        return this.persistence
            ...
            .then(queryData => {
            assert(this.queryDataByTarget.get(queryData.targetId) === null, 'Tried to allocate an already allocated query: ' + query);
            this.queryDataByTarget = this.queryDataByTarget.insert(queryData.targetId, queryData);
    ...
    getQueryData(transaction, query) {
        ...
        if (targetId !== undefined) {
            return PersistencePromise.resolve(this.queryDataByTarget.get(targetId));
        }
        else {
    ...
    releaseQuery(query, keepPersistedQueryData) {
        ...
        return this.persistence
            ...
            assert(cachedTargetId !== undefined, 'Tried to release nonexistent query: ' + query);
    ...
    collectGarbage(garbageCollector) {
        return this.persistence.runTransaction('Collect garbage', 'readwrite-primary-idempotent', txn => garbageCollector.collect(txn, this.queryDataByTarget));
    }
    // PORTING NOTE: Multi-tab only.
    getQueryForTarget(targetId) {
        ...
        if (cachedQueryData) {
            ...
            return this.persistence.runTransaction('Get query data', 'readonly-idempotent', txn => {
                return this.queryCache
